...
HRESULT Direct3D::InitToonShader()
{
	...
	ID3DBlob* pCompileVS = nullptr;
	//                    ファイル名　　　　　　　　　　　　バージョン
	D3DCompileFromFile(L"ToonShader.hlsl", nullptr, nullptr, "VS", "vs_5_0", NULL, 0, &pCompileVS, NULL);//コンパイルするよ
	assert(pCompileVS != nullptr);//true
	hr = pDevice_->CreateVertexShader(pCompileVS->GetBufferPointer(),
...
HRESULT Direct3D::InitNormalMap()
{
	...
	ID3DBlob* pCompileVS = nullptr;
	//                    ファイル名　　　　　　　　　　　　バージョン
	D3DCompileFromFile(L"NormalMapping.hlsl", nullptr, nullptr, "VS", "vs_5_0", NULL, 0, &pCompileVS, NULL);//コンパイルするよ
	assert(pCompileVS != nullptr);//true
	hr = pDevice_->CreateVertexShader(pCompileVS->GetBufferPointer(),
		pCompileVS->GetBufferSize(), NULL, &(shaderBundle[SHADER_NORMALMAP].pVertexShader_));
	if (FAILED(hr))
	{
		//失敗したときの処理
		MessageBox(nullptr, "頂点シェーダの作成に失敗しました", "エラー", MB_OK);
	...
	hr = pDevice_->CreateInputLayout(layout, ARRAYSIZE(layout), pCompileVS->GetBufferPointer(),
